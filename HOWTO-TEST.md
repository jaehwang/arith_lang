...
### 문제 상황
- 정상 케이스만 테스트하여 파서의 문제를 발견하지 못함
- 실제로는 잘못된 구문(`x = 1 y = 2`)도 허용하는 파서가 만들어짐
- 실행 방식 정의가 없는 구문(`x = "hello" + 2`)도 허용하는 파서가 만들어짐
- negative 테스트 부재로 인해 파서의 오류 검증 능력을 확인할 수 없었음
### 해결 과정

1. **체계적 negative 테스트 구축**
   - 상세한 테스트 사양서 작성 (참조: [@specs/test_parser_negative_cases.md](specs/test_parser_negative_cases.md). AI에게 간단한 프롬프트로 negative case 생성 요청 후 사람이 검토 및 수정하여 완성함)
